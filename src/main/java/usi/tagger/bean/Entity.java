...
public class Entity implements Comparable<Entity>, Serializable {
    ...
    public boolean isFalsePositive() {
        ...
        if (maxValue > 1 && ((double) maxValue / overallEvaluations) >= 0.5
                && (maxType.toLowerCase().equals("false positive") || maxType.toLowerCase().equals("generic")
                        || maxType.toLowerCase().equals("unclear")))
            // || maxType.toLowerCase().equals("unclear"))) ||
            // ((double)falsePostivePlusUnclear/overallEvaluations) > 0.5))
            return true;
